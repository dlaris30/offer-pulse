---
name: overnight
description: Autonomous improvement loop. Runs freshness checks → stable-only batch test → targeted Wendy fix (if needed) → surface vocab expansion → branch map → offer matrix → package catalog. Designed for unattended overnight runs. Does NOT run tests on structurally unreliable cases.
---

# /overnight — Autonomous Improvement Engine

## Agent Mode (run in background)

When invoked as a skill (`/overnight`), spawn a background agent rather than executing phases inline.

1. Parse `$ARGUMENTS` for `max_fix_cycles` (optional integer, default: 1, max: 2).
2. Use the Agent tool:
   - `subagent_type`: `general-purpose`
   - `run_in_background`: `true`
   - `description`: `"Overnight improvement loop (max_fix_cycles: {N})"`
   - `prompt`: Read this SKILL.md and pass everything from **## Design Principles** onward — verbatim — as the agent's prompt. The agent executes all phases independently.
3. Confirm immediately (do not wait for the agent):
   ```
   /overnight agent spawned — max_fix_cycles: {N}
   Phases 1–5 will run unattended. You'll be notified when complete.
   ```

The agent has access to all Redshift, catalog MCP, and file editing tools. No mid-cycle user confirmation will occur.
...
**Invoking this skill is consent to edit SKILL.md, use-case-log.md, tree.md, surface-vocab.md, matrix.md, and catalog.md unattended.** No mid-cycle confirmation prompts will be issued.

---

## Design Principles

The core problem this skill solves is different from what the old overnight skill tried to solve. The old design treated all batch-test failures as skill defects. They are not. Some failures are caused by structurally unreliable test cases — WebFetch truncation, A/B surface ambiguity, champion rotation, Jira ticket body routing sensitivity. Fixing the skill against those failures degrades it.
...
**Current volatile set (excluded from overnight batch test):**
...
{One of:}
  ...
  Stable cases failed; Wendy found no actionable fix — log to /system-gaps.
...
- Each phase header must be visible — it is the progress marker for unattended runs
...
**golden-set:** Add `reliability` as a required field in the entry schema. Default: `volatile` for CES entries, `stable` for NES entries where analyst confirms single-champion. The `/golden-set add` and `/golden-set promote` flows should ask for `reliability` when creating a new entry. Existing entries without this field are treated as `volatile` by overnight and batch-test.
